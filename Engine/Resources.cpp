...
shared_ptr<MeshData> Resources::LoadFBX(const wstring& path)
{
	...
	if (meshData)
		return meshData;	// 이미 로드된 메시라면 반환

	meshData = MeshData::LoadFromFBX(path);	// 여기서 오래 걸림.
...
void Resources::CreateDefaultMaterial()
{
	...
	// Terrain
	{
		...
		shared_ptr<Texture> texture = GET_SINGLE(Resources)->Load<Texture>(L"Terrain", L"..\\Resources\\Texture\\Terrain\\rock_face_03_diff_4k.jpg");
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		material->SetTexture(0, texture);
