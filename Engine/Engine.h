...
public:
	...
	shared_ptr<GraphicsCommandQueue>	GetGraphicsCmdQueue()							{ return graphicsCmdQueue; }
	shared_ptr<ComputeCommandQueue>		GetComputeCmdQueue()							{ return computeCmdQueue; }
	shared_ptr<SwapChain>				GetSwapChain()									{ return swapChain; }
	shared_ptr<RootSignature>			GetRootSignature()								{ return rootSignature; }
	shared_ptr<GraphicsDescriptorHeap>	GetGraphicsDescHeap()							{ return graphicsDescHeap; }
	shared_ptr<ComputeDescriptorHeap>	GetComputeDescHeap()							{ return computeDescHeap; }
...
private:
	...
	shared_ptr<GraphicsCommandQueue>	graphicsCmdQueue	= make_shared<GraphicsCommandQueue>();
	shared_ptr<ComputeCommandQueue>		computeCmdQueue		= make_shared<ComputeCommandQueue>();
	shared_ptr<SwapChain>				swapChain			= make_shared<SwapChain>();
	shared_ptr<RootSignature>			rootSignature		= make_shared<RootSignature>();
	shared_ptr<GraphicsDescriptorHeap>	graphicsDescHeap	= make_shared<GraphicsDescriptorHeap>();
	shared_ptr<ComputeDescriptorHeap>	computeDescHeap		= make_shared<ComputeDescriptorHeap>();
