...
ParticleSystem::ParticleSystem() : Component(COMPONENT_TYPE::PARTICLE_SYSTEM)
{
	...
	shared_ptr<Texture> tex = GET_SINGLE(Resources)->Load<Texture>(L"bubble", L"..\\Resources\\Texture\\Particle\\bubble.png");
...
BulletParticle::BulletParticle() : Component(COMPONENT_TYPE::PARTICLE_SYSTEM)
{
	_particleBuffer = make_shared<StructuredBuffer>();
	_particleBuffer->Init(sizeof(ParticleInfo), _maxParticle);
	...
	SetParticleShader(L"Particle");
	SetParticleTexture(L"laser", L"..\\Resources\\Texture\\Particle\\laser.png");
...
void BulletParticle::FinalUpdate()
{
	_accTime += DELTA_TIME;
...
void BulletParticle::SetParticleShader(const wstring& name)
{
		_material = GET_SINGLE(Resources)->Get<Material>(name);
}

void BulletParticle::SetParticleTexture(const wstring& name, const wstring& path)
{
		shared_ptr<Texture> tex = GET_SINGLE(Resources)->Load<Texture>(name, path);
	_material->SetTexture(0, tex);
}
