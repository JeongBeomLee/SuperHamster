...
void Mesh::CreateFromMeshData(const vector<Vertex>& vertexBuffer, const vector<vector<uint32>>& indexBuffer)
{
	CreateVertexBuffer(vertexBuffer);
...
//void Mesh::CreateBoneOffsetBuffer(uint32 elementSize, uint32 elementCount, void* initialData)
//{
//	_offsetBuffer = make_shared<StructuredBuffer>();
//	_offsetBuffer->Init(elementSize, elementCount, initialData);
//}
...
#pragma region AnimClip
	...
	for (shared_ptr<FbxAnimClipInfo>& ac : animClips) {
		...
		info.animName = ac->name;	// 애니메이션 이름
		info.duration = ac->endTime.GetSecondDouble() - ac->startTime.GetSecondDouble(); // 애니메이션 길이
		info.frameCount = endFrame - startFrame;		// 프레임 수
		info.keyFrames.resize(ac->keyFrames.size());	// 뼈 개수

		const int32 boneCount = static_cast<int32>(ac->keyFrames.size());	// 뼈 개수
		for (int32 b = 0; b < boneCount; b++) {
			auto& vec = ac->keyFrames[b];

			const int32 keyFreamInfoCount = static_cast<int32>(vec.size());
			//frameCount = max(frameCount, static_cast<uint32>(keyFreamInfoCount));
			info.keyFrames[b].resize(keyFreamInfoCount);

			for (int32 f = 0; f < keyFreamInfoCount; f++) {
				FbxKeyFrameInfo& kf = vec[f];
				// FBX에서 파싱한 정보들로 채워준다
				KeyFrameInfo& kfInfo = info.keyFrames[b][f];
				kfInfo.time = kf.time;
				kfInfo.frame = static_cast<int32>(keyFreamInfoCount);
...
#pragma region SkinData
	if (IsAnimMesh())
	{
		...
		for (int32 i = 0; i < animCount; ++i) {
			...
			const size_t expectedSize = _bones.size() * animClip.frameCount;
			frameParams.reserve(expectedSize);
			...
			for (int32 b = 0; b < boneCount; b++) {
				...
				for (int32 f = 0; f < keyFrameCount; f++) {
					...
					if (idx >= frameParams.size()) {
						// idx가 frameParams의 현재 크기를 넘어서는 경우, 크기를 조정합니다.
						frameParams.resize(idx + 1); // idx에 맞게 크기를 조정합니다.
					}

					// 이제 안전하게 값을 할당할 수 있습니다.
